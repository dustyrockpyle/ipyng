(function(angular){
  ...
  angular.module('ipyng.utils', [])
    .factory('$ipyUtils', ipyUtilsFactory)
    .directive('resizable', resizableDirective);

  function ipyUtilsFactory () {
    var $ipyUtils = {
    ...
    function to_absolute_cursor_pos (cm, cursor) {
    ...
    function from_absolute_cursor_pos (cm, cursor_pos) {
     ...
     * // -> ['..', 'word', '1', ' ', 'word', '2', '..']
     */
    function regex_split (str, separator, limit) {
      var output = [],
    ...
    function always_new (constructor) {
      /**
  ...
  function resizableDirective ($timeout){
    return {
      replace: true,
      restrict: 'A',
